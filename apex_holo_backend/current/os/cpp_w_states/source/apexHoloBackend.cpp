...
apexHoloBackend::apexHoloBackend(const std::string &binfile, const std::string &dev_mem,
    ...
    std::cout << "programming the FPGA with "<<binfile << "\n";
    std::string helper = "fpgautil -b "+binfile;
    if(system(helper.c_str()))
    ...
    //        }
    sleep(1);
    std::cout << "openinig the "<< dev_mem  <<" file..";
    ...
    std::cout << "reseting the system and enabling adc..";
    *(axil_reg_intf) = 1;
    *(axil_reg_intf) = 0;
    *(axil_reg_intf) = 2;
    std::cout << "done\n";
...
int apexHoloBackend::get_ring_buffer_data(std::vector<int64_t> &ring_data){
    ...
    this->ring_buffer_read_pointer = index;
    return 0;
}
