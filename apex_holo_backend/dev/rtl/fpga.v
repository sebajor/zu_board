...
//put user inputs outputs  TODO!!
    ...
    .disable_ring_buff(disable_ring_buff),
    //inputs
...
wire disable_ring_buff_r;

delay #(
    .DATA_WIDTH(1),
    .DELAY_VALUE(10)
) ring_disable_synchronizer (
    .clk(mpsoc_clk_100),
    .din(disable_ring_buff),
    .dout(disable_ring_buff_r)
);



wire ring_valid = (|corr_dout_valid_r) & (~disable_ring_buff_r);

always@(posedge data_clk_div)begin
    if(reset)
        bram_counter <=0;
    else if(ring_valid)begin
