...
class correlator_1bin_client():
    ...
    def get_power_data(self):
        msg = "ZUBOARD:GET_POWER"
        self.client.send(msg.encode())
        time.sleep(self.sleep)
        ans = self.client.recv(2**15)
        data = np.frombuffer(ans, 'Q')
        power0 = data[:len(data)//2]
        power1 = data[len(data)//2:]
    ...
    def get_corr_data(self):
        ...
        ans = self.client.recv(2**15)
        data = np.frombuffer(ans, 'q')
        corr_re= data[:len(data)//2]
        corr_im = data[len(data)//2:]
